...
Every command currently in this skill is verified, against `gh` 2.86.0. The mark stays on
each one so that a command added later has to earn it rather than inherit it.
...
- `gh api graphql --input <file>` sends a request too complex for flags. The file holds
  `{"query":"mutation(...){...}","variables":{...}}` — query and variables together, not a
  bare variables object.
...
- `--merge` is the `--no-ff` equivalent: it always writes a merge commit. `-t`/`-b` set its
  subject and body, and apply to `--merge` and `--squash` only.
- The merge runs server-side against the pushed branch. A local rebase that has not been
  ...
  by the local git identity — this repository's merge commits were produced this way.
- Repository settings it depends on: "Allow merge commits" ON, "Require linear history"
  OFF. The second one rejects a merge commit outright.
...
All four commands below are **[verified]** against a live PR; the publishing behaviour in
...
1. The pending path exists only over GraphQL. `gh pr review` submits on the spot, and REST
   `in_reply_to` has no pending form at all.
2. One pending review per user per PR. A second `addPullRequestReview`, and the REST
   equivalent, answer `User can only have one pending review per pull request` — look for
   an open one and reuse it rather than creating a second.
3. A typed GraphQL variable cannot go through `-f`/`-F`: the `threads` list on
   `addPullRequestReview` is a list of input objects, and both flags produce scalars. Add
   threads one at a time with `addPullRequestReviewThread`, or send the whole request with
   `gh api graphql --input <file>` (see Conventions).
4. `pullRequestReviewId` is *optional* on `addPullRequestReviewThreadReply`. A reply
   without it belongs to no pending review, which makes its absence a publish.
