...
// The lexer read against the shell over a generated space rather than a written list. A
// hand-picked list covers the mechanisms somebody thought of; a grammar covers the
// combinations nobody wrote, and a new mechanism is added to it as a dimension rather than
// as another example.
//
...
// The bypasses this lexer is known to have, each pinned inverted against the class it
// belongs to: the shape asserts that bash still runs what the lexer still misses, so it
// goes red both if the bypass regresses further and when it starts being read correctly.
// Naming a class without pinning a witness of it costs that ratchet. A shape going red
// here is the signal to drop it from this list rather than to narrow the space.
...
// wrapper there is, not an adversarial spelling.
const RESERVED_WORD = 'a reserved word is not a separator';
const KNOWN_MISSING = new Map([
  ['A wrapper: brace/glued first', RESERVED_WORD],
  ['A wrapper: brace/glued first +prefix', RESERVED_WORD],
  ['E reserved: then', RESERVED_WORD],
  ['E reserved: do for', RESERVED_WORD],
  ['E reserved: do while', RESERVED_WORD],
  ['E reserved: until', RESERVED_WORD],
  ['E reserved: else', RESERVED_WORD],
  ['E reserved: elif', RESERVED_WORD],
  ['E reserved: bang', RESERVED_WORD],
  ['E reserved: function body', RESERVED_WORD],
  ['E reserved: select', RESERVED_WORD],
  ['E reserved: coproc', RESERVED_WORD],
]);
...
for (const [index, script] of indexed) {
  ...
  if (KNOWN_MISSING.has(id)) {
    test(`known missing, ${KNOWN_MISSING.get(id)} - ${id}`, { skip }, () => {
