...
- This skill covers how modules/services/processes fit together. A single public
  interface's own hygiene (not what a reuse may expose across a public boundary — that
  stays here, under Common Tradeoff Axes) → the API-design skill of the language being
  written.
...
- **Build vs buy/reuse** — building gives full authority over the implementation and a
  ...
  surface of the boundary under discussion — the surface consumers outside it compile,
  link or import against — and nothing at all crosses it where every consumer sits
  inside the same build; a deliberate exposure is a cost weighed under Quality
  Priorities step 4.
