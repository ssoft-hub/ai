...
// Keys that hold a single value rather than a list: install overwrites them and records
// what was there, since a merge has nothing to append to.
const REPLACED_KEYS = ['statusLine'];

function mergeUnique(existing, incoming) {
  return [...new Set([...existing, ...incoming])];
}

// Append the incoming hook entries whose command is not already present, and report
// every incoming command as install-owned: each launches a dispatcher uninstall removes.
function mergeHookEvent(existing, incoming) {
  ...
  for (const entry of incoming) {
    ...
    const newHooks = hooks.filter(h => !existingCmds.has(h.command));
    if (newHooks.length) result.push({ ...entry, hooks: newHooks });
    for (const h of hooks) if (h.command) addedCommands.push(h.command);
  }
...
function mergeSettings(existing, repo) {
  ...
  const additions = { hooks: {}, permissions: {}, replaced: {} };
  ...
  for (const key of REPLACED_KEYS) {
    ...
    additions.replaced[key] = key in out
      ? { had: true, prior: structuredClone(out[key]) }
      : { had: false };
    out[key] = structuredClone(repo[key]);
  }
...
function additionsFromRepo(repo) {
  const additions = { hooks: {}, permissions: {}, replaced: {} };
  for (const key of REPLACED_KEYS)
    if (repo[key] !== undefined) additions.replaced[key] = { had: false };
  for (const [ev, entries] of Object.entries(repo.hooks || {})) {
    const cmds = [];
...
function subtractAdditions(settings, additions = {}) {
  ...
  for (const [key, record] of Object.entries(additions.replaced || {})) {
    if (record.had) settings[key] = record.prior;
