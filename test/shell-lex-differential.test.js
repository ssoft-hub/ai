...
// One bash process per shape, so the corpus is opt-in locally and set for every CI cell.
const skip = process.env.SHELL_LEX_CORPUS
  ? bashMissing()
  : 'set SHELL_LEX_CORPUS=1 to read the lexer against bash';
...
const REACH = [
  ...
  },
  {
    ...
    // an interpreter into what that interpreter runs.
    nested: false,
  },
];
...
// A here-string handed to an interpreter is a program, and one handed to a reader of data
// is not. Both take the token path a here-document does not.
const HERE_STRING = [
  ['bash runs a here-string', `bash <<<'${PAYLOAD}'`],
  ['sh runs a here-string', `sh <<<'${PAYLOAD}'`],
  ['bash runs a here-string built by a substitution', `bash <<<"$(printf '%s' '${PAYLOAD}')"`],
  ['cat only writes a here-string', `cat <<<'${PAYLOAD}'`],
];
...
const GROUPS = [['no here-document', NO_HEREDOC], ['comment contents', COMMENT_CONTENTS],
  ...
  ['interpreter body', INTERPRETER_BODY], ['here-string', HERE_STRING]];
...
// The bypasses this lexer is known to have, keyed on the shape's id and pinned inverted:
// the pin asserts bash still runs what the lexer still misses, so it goes red when the
// shape starts being read correctly. Naming a class without pinning a witness of it costs
// that ratchet. A pin going red is the signal to drop it from this list rather than to
// drop the shape from the corpus.
const COMMENT = 'a # does not start a comment, so its text is lexed as source';
const GLUED_QUOTE = 'a nested source spelled with glued quoting is not read as one';
const UNREAD_HERE_STRING = 'an interpreter here-string is not read as a program';
const ESCAPE = 'a backslash does not take the shell meaning from the character after it';
const ARITHMETIC = 'an arithmetic expansion is lexed as the parens it is spelled with';
const KNOWN_MISSING = new Map([
  ['comment contents: comment holding an apostrophe', COMMENT],
  ['comment contents: comment holding a double quote', COMMENT],
  ['comment contents: comment ending in >', COMMENT],
  ['comment contents under bash -c: comment ending in >', COMMENT],
  ['comment contents: comment ending in <<', COMMENT],
  ['comment contents under bash -c: comment ending in <<', COMMENT],
  ['comment contents: comment ending in 2>', COMMENT],
  ['comment contents under bash -c: comment ending in 2>', COMMENT],
  ['comment contents: comment ending in <', COMMENT],
  ['comment contents under bash -c: comment ending in <', COMMENT],
  ['gitCalls sees what bash runs - after a comment holding an apostrophe', COMMENT],
  ['writeTargets sees what bash runs - after a comment holding an apostrophe', COMMENT],
  ['interpreter body under bash -c: bash reads a quoted-marker body', GLUED_QUOTE],
  ['here-string: bash runs a here-string', UNREAD_HERE_STRING],
  ['here-string under bash -c: bash runs a here-string', UNREAD_HERE_STRING],
  ['here-string: sh runs a here-string', UNREAD_HERE_STRING],
  ['here-string under bash -c: sh runs a here-string', UNREAD_HERE_STRING],
  ['here-string: bash runs a here-string built by a substitution', UNREAD_HERE_STRING],
  ['here-string under bash -c: bash runs a here-string built by a substitution', UNREAD_HERE_STRING],
  ['word splitting: a\\;b', ESCAPE],
  ['word splitting: a\\ b', ESCAPE],
  ['word splitting: a\\\\nb', ESCAPE],
  ['word splitting: $((1+1))#x', ARITHMETIC],
  ['word splitting: $((1 << 3))', ARITHMETIC],
  ['word splitting: a$((1))b c', ARITHMETIC],
]);
...
function differential(id, shape) {
  const reason = KNOWN_MISSING.get(id);
  if (reason) {
    test(`known missing, ${reason} - ${id}`, { skip }, () => {
      assert.ok(ranUnderBash(shape) && !lexSees(shape),
...
for (const [group, shapes] of GROUPS) {
  for (const [name, shape] of shapes) {
    ...
    differential(`${group} under bash -c: ${name}`, `bash -c ${singleQuoted(shape)}`);
  }
}
...
for (const { guard, payload, ran, seen, nested } of REACH) {
  for (const [name, wrap, needsNesting] of [
    ...
    const id = `${guard} sees what bash runs - ${name}`;
...
const lexedWordCount = args => lex(`echo ${args}`).filter(token => token.word !== undefined).length - 1;

for (const args of WORD_SPLITTING) {
  const id = `word splitting: ${args.replace(/\n/g, '\\n')}`;
  const reason = KNOWN_MISSING.get(id);
  if (reason) {
    test(`known missing, ${reason} - ${id}`, { skip }, () => {
      assert.notStrictEqual(lexedWordCount(args), shellWordCount(args),
        `this argument is now split as the shell splits it — drop it from KNOWN_MISSING`);
    });
    continue;
  }
  test(`word splitting matches the shell: ${args.replace(/\n/g, '\\n')}`, { skip }, () => {
    assert.strictEqual(lexedWordCount(args), shellWordCount(args));
  });
}
