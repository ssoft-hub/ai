...
// The owner is derived as the one skill carrying these headings.
...
// A citation inside a fenced block is a quotation.
...
function citations(text) {
  const flat = unfenced(text).replace(/\s+/g, ' ');
...
function mapSkillNames(mapText) {
...
test('citations reads nothing from a citation inside a fenced block', () => {
  assert.deepStrictEqual(citations(`\`\`\`markdown\n\`editing\` → ${SECTION}\n\`\`\`\n`), []);
});
...
test('mapSkillNames reads the name opening each bullet, a wrapped bullet whole', () => {
  const map = '- `artifact-placement` — where an artifact a rule\n' +
    '  produces is put.\n- `comments` — a comment in any language.\n';
  assert.deepStrictEqual(mapSkillNames(map), ['artifact-placement', 'comments']);
});

test('mapSkillNames reads no bullet opening with something other than a name', () => {
  assert.deepStrictEqual(mapSkillNames('- a line naming no skill.\n'), []);
});

test('the ownership map carries exactly one line for the skill stating the statement', () => {
  const owner = owningSkill();
  const opened = mapSkillNames(ownershipMap()).filter(name => name === owner);
  assert.strictEqual(opened.length, 1,
    `the ownership map opens ${opened.length} bullets with \`${owner}\``);
});
