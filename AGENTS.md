...
table, and the one keyed on `Skill` the criteria table:

| Stated of the skill | Where it stands |
|---|---|
| its name alone, or its name and the clause conditioning it | a `Skills` cell of the stage table |
| its input, its output, and the artifact its entry and its exit turn on | a row of the criteria table |

A skill may stand in both. The `Stage` cell of a criteria row names the
...
skill whose `Stage` cell reads `cross-cutting` as well. No cell of the criteria table
states a trigger.

| Skill | Stage | Input | Output | Entry criterion | Exit criterion |
|-------|-------|-------|--------|------------------|-----------------|
| `artifact-placement` | cross-cutting | the artifact, and the conventions the project states for a record of its kind | the location the artifact is written to | the requirement `artifact-placement` → The Requirement, Not the Path names for a location | the location traced to a stated convention or to an answer, per `artifact-placement` → Where the Place Comes From |
...
An `Exit criterion` cell may name an artifact another stage or another skill produces, as
the rows for `hook-scripts`, `node-testing`, `skill-authoring` and
`observability-and-instrumentation` do: what it fixes is what ends the skill's own work,
not who produces that artifact or where it is used.
